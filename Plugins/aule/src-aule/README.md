...
- **Self-improving plugin ecosystem:** Trashbot automatically keeps every plugin current as standards evolve. When Aulë improves, the entire ecosystem follows — no manual updates, no plugin drift.Let's
...
- **plugin-generation** — Translates discovery output into a validated `PluginSpec` JSON object for the compiler. Translation layer only — does not write plugin files directly.
...
- **plugin-builder** — End-to-end orchestrator that manages the 5-phase plugin creation workflow: discovery → spec translation → validation → compile → delivery
...
**Key structure:**
- Each plugin has a **parent folder** (e.g., `Plugins/aule/`) that acts as the workspace
- Inside: **`src-[plugin-name]/`** (the actual plugin source code) and **`[plugin-name].plugin`** (the packaged zip)
- The **`src-[plugin-name]/` folder** contains all plugin files (skills, agents, commands, .claude-plugin/, README, etc.)
- The **`.plugin` file is a sidecar** in the parent folder, NOT inside `src-[plugin-name]/`
