...
- Schema files in `contracts/schemas/` are read at runtime — not from memory or training
...
- `contracts/schemas/telemetry-event-schema.json` — how to log permission_gate and verification_gate events
- `contracts/schemas/plugin-identity-schema.json` — what fields validate plugin identity at the source-of-truth layer
